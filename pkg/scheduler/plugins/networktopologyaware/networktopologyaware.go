...
func (nta *networkTopologyAwarePlugin) OnSessionOpen(ssn *framework.Session) {
	...
	hyperNodeFn := func(job *api.JobInfo, hyperNodes map[string][]*api.NodeInfo) (map[string]float64, error) {
		...
		for hyperNode := range hyperNodes {
			score := networkTopologyAwareScore(hyperNode, job, ssn.HyperNodesTiers, ssn.HyperNodesSetByTier, ssn.HyperNodesDescendants)
		...
		if len(scoreHyperNode[maxScore]) > 1 {
			for hyperNode, score := range hyperNodeScores {
				if score == maxScore {
					taskNumScore := networkTopologyAwareScoreWithTaskNum(hyperNode, job, ssn.RealNodesList)
	...
	nodeFn := func(task *api.TaskInfo, nodes []*api.NodeInfo) (map[string]float64, error) {
		...
		for _, node := range nodes {
			hyperNode := util.FindHyperNodeOfNode(node.Name, ssn.RealNodesList)
			score := networkTopologyAwareScore(hyperNode, taskJob, ssn.HyperNodesTiers, ssn.HyperNodesSetByTier, ssn.HyperNodesDescendants)
			score *= float64(weight)
		...
		if len(scoreNodes[maxScore]) > 1 {
			for node, score := range nodeScores {
				if score == maxScore {
					hyperNode := util.FindHyperNodeOfNode(node, ssn.RealNodesList)
					taskNumScore := networkTopologyAwareScoreWithTaskNum(hyperNode, taskJob, ssn.RealNodesList)
					taskNumScore *= float64(weight)
...
func networkTopologyAwareScore(hyperNodeName string, job *api.JobInfo, hyperNodesTiers []int, hyperNodesListByTier map[int]sets.Set[string], hyperNodesDescendants map[string]sets.Set[string]) float64 {
	jobHyperNode := job.PodGroup.GetAnnotations()[api.TopologyAllocateLCAHyperNode]
	...
	_, index := util.FindLCAHyperNode(hyperNodeName, jobHyperNode, hyperNodesTiers, hyperNodesListByTier, hyperNodesDescendants)
	if index <= 0 {
		klog.V(4).Infof("find LCAhyperNode failed wtih %s in hyperNodeTree", hyperNodeName)
		return 0.0
	}
	tierIndexScore := BaseScore * scoreHyperNodeWithIndex(index, 1, len(hyperNodesListByTier))
