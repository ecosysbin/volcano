...
func TestNetworkTopologyAwareHyperNodeScore(t *testing.T) {
	tests := []struct {
		...
		HyperNodesTiers       []int
		HyperNodesDescendants map[string][]string
		jobHyperNode          string
	...
	}{
		{
			...
			jobHyperNode: "",
			HyperNodesListByTier: map[int][]string{
				1: {"hyperNode3", "hyperNode4", "hyperNode5", "hyperNode6"},
				2: {"hyperNode1", "hyperNode2"},
				3: {"hyperNode0"},
			},
			HyperNodesTiers: []int{1, 2, 3},
			HyperNodesDescendants: map[string][]string{
				"hyperNode3": {"hyperNode3"},
				"hyperNode4": {"hyperNode4"},
				"hyperNode5": {"hyperNode5"},
				"hyperNode6": {"hyperNode6"},
				"hyperNode1": {"hyperNode1", "hyperNode3", "hyperNode4"},
				"hyperNode2": {"hyperNode2", "hyperNode5", "hyperNode6"},
				"hyperNode0": {"hyperNode0", "hyperNode1", "hyperNode2", "hyperNode3", "hyperNode4", "hyperNode5", "hyperNode6"},
			},
			expected: map[string]float64{
				"hyperNode3": 0.0,
				"hyperNode4": 0.0,
	...
	for i, test := range tests {
		...
		ssn.RealNodesList = test.hyperNodes
		// mock for test
		ssn.HyperNodesTiers = test.HyperNodesTiers
		ssn.HyperNodesSetByTier = map[int]sets.Set[string]{}
		for index, hyperNodes := range test.HyperNodesListByTier {
			ssn.HyperNodesSetByTier[index] = sets.New[string]()
			for _, hyperNode := range hyperNodes {
				ssn.HyperNodesSetByTier[index] = ssn.HyperNodesSetByTier[index].Insert(hyperNode)
			}
		}
		ssn.HyperNodesDescendants = map[string]sets.Set[string]{}
		for hyperNode, descendants := range test.HyperNodesDescendants {
			ssn.HyperNodesDescendants[hyperNode] = sets.New[string]()
			for _, descendant := range descendants {
				ssn.HyperNodesDescendants[hyperNode] = ssn.HyperNodesDescendants[hyperNode].Insert(descendant)
			}
		}
		job := &api.JobInfo{
			Name:     "test-job",
		...
		scores, err := ssn.HyperNodeOrderMapFn(job, ssn.RealNodesList)
		if err != nil {
...
func TestNetworkTopologyAwareNodeScore(t *testing.T) {
	...
	for i, test := range tests {
		...
		ssn.RealNodesList = test.hyerNodes
		job.Tasks = make(map[api.TaskID]*api.TaskInfo)
		for name, node := range test.tasks {
			taskInfo := &api.TaskInfo{
				UID:  api.TaskID(name),
